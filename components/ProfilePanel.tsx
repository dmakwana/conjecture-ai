...
import type { ColumnProfile, TableProfile } from "@/lib/profile/types";
...
export function ProfilePanel({ profile }: { profile: TableProfile }) {
  ...
  return (
    <section className="panel rounded-md">
      <header className="flex items-baseline justify-between gap-4 px-4 py-3 border-b hairline flex-wrap">
        <div>
          <span className="font-medium">{profile.table}</span>
          <span className="muted text-sm ml-2">
            {profile.rowCount.toLocaleString()} rows · {profile.columnCount} columns ·{" "}
            {profile.format} · profiled in {profile.profileMs} ms
          </span>
        </div>
        <div className="flex gap-4 text-sm">
          <button onClick={() => setShowColumns((v) => !v)} className="underline underline-offset-2 hover:no-underline">
            {showColumns ? "hide columns" : "columns"}
          </button>
          <button onClick={() => setShowPayload((v) => !v)} className="underline underline-offset-2 hover:no-underline">
      ...
      {showPayload && (
        <div className="px-4 py-3 border-t hairline">
          <p className="muted text-sm mb-2">
            ...
            column names, type names and numbers, plus masked format shapes where every letter
            is <code className="font-mono">a</code>/<code className="font-mono">A</code> and every
            digit is <code className="font-mono">9</code>. No cell values.
          </p>
